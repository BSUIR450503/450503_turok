...
extern "C" {
	...
	__declspec(dllexport) BOOL read(IncomingInformation *information) {
		...
		WaitForSingleObject(information->Overlapped.hEvent, INFINITE); // ждем завершения операции
		readInfo = GetOverlappedResult(information->hFile, &information->Overlapped, &number_of_trasferred_bytes, FALSE); // != 0 если нет ошибки
	...
	int read_async(struct IncomingInformation *information) {
		...
		while (aio_error(&information->aiocbStruct) == EINPROGRESS); //ждем завершения операции
		information->number_of_trasferred_bytes = aio_return(&information->aiocbStruct); // прочитано байт
