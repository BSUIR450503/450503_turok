...
Node::Node(char *fileName, int size, int start) {
...
void Node::createNewFile(char *fileName, int sectors, int start) {
	nextFileOrFolder[branches] = new Node(fileName, sectors, start);
	branches++;
}

void Node::createNewFolder(char *folderName) {
	nextFileOrFolder[branches] = new Node(folderName);
	branches++;
}

void Node::defragmentation(int start, int size) {
	if (fileOrFolder == _FOLDER) {
		if (branches > 0) {
			for (int i = 0; i < branches; i++) {
...
void Node::deleteFileOrFolder(char *fileOrFolderName) {
	bool flag = false;
...
Node *Node::moveToNextFolder(char *folderName) {
	bool flag = false;
...
char *Node::getItemName() {
	return itemName;
}

void Node::copyFileFromRealSystem(char *fileName) {
	char buffer;
	int clearSectors = 0;
	int clearBytes = 0;
	...
	nextFileOrFolder[branches] = new Node(fileName, inputFileSize, diskSize);
	branches++;
...
void Node::copyFileFromVirtualSystem(char *fileName) {
	bool flag = false;
	char buff;
