...
void loadFolderFromFile(FILE *vitrualDiskTree, Node **current, int depth) {
...
void loadDataFormFile(Node **current) {
	int depth = 0;
...
void help() {
	cout << endl << "dir			-- show directory structure";
	cout << endl << "cd 'name'      -- move to next folder";
	cout << endl << "cd ..          -- move to previous folder";
...
void openVirtualDisk() {
	...
	Node *current[MAX_FOLDERS_DEPTH];
	current[depth] = new Node("root");
